...
async function load() {
  ...
  const data = await api(`/quizzes?page=${page}&limit=${limit}&search=${q}`, { auth: false });
  list.innerHTML = '';
  data.items.forEach((q) => {
    ...
    div.innerHTML = `<h3>${q.title}</h3>
      <p>${q.description || ''}</p>
      <p class="muted">${q.question_count} questions · ${q.duration_seconds}s · by ${q.author}</p>
      <button>Start</button>`;
    div.querySelector('button').onclick = () => {
      sessionStorage.setItem('startQuizId', q.id);
...
(async () => {
  ...
  try {
    const { items } = await api('/attempts/me');
    items.forEach((a) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td>${a.quiz_title}</td><td>${a.status}</td>
